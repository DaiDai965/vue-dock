import type { DragType } from "./DragManager";

export type GestureHandler = (event: PointerEvent) => void;
...
export class GestureState {
  ...
  constructor(
    ...
    public component: GestureComponent,
    public _init = false,
  ) {
    ...
    if (this.dx1 * this.dx2 >= 0) this.dx = (this.dx1 + this.dx2) / 2;
    if (this.dy1 * this.dy2 >= 0) this.dy = (this.dy1 + this.dy2) / 2;

    const distance = Math.hypot(touch2.pageX - touch1.pageX, touch2.pageY - touch1.pageY);
    this.scale = component.baseDis ? distance / component.baseDis : 1;
    this.rotate = Math.atan2(touch2.pageY - touch1.pageY, touch2.pageX - touch1.pageX) - component.baseAng;

    if (this.rotate > Math.PI) this.rotate -= Math.PI * 2;
    else if (this.rotate < -Math.PI) this.rotate += Math.PI * 2;
  }
...
export class GestureManager {
  private moveHandlers = new Set<GestureHandler>();
  private endHandlers = new Set<GestureHandler>();

  onMove(handler: GestureHandler) {
  ...
  emitMove(event: PointerEvent) {
    for (const handler of this.moveHandlers) handler(event);
  }

  emitEnd(event: PointerEvent) {
    for (const handler of this.endHandlers) handler(event);
  }
}